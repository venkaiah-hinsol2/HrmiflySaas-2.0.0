<template>
    <div v-if="permsArray.includes('attendances_create') || permsArray.includes('admin')">
        <template v-if="customType == 'menu'">
            <a-menu-item @click="showAdd" key="attendances">
                <template #icon>
                    <slot name="icon"></slot>
                </template>
                <slot></slot>
        ...
        <AddEdit
            ...
            :formData="formData"
            :data="formData"
            :pageTitle="$t('attendance.add')"
            :successMessage="$t('attendance.created')"
        />
    </div>
</template>
<script>
