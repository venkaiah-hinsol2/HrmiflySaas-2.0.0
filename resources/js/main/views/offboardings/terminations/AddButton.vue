<template>
    <div v-if="permsArray.includes('terminations_view') || permsArray.includes('admin')">
        <template v-if="customType == 'menu'">
            <a-menu-item @click="showAdd" key="terminations">
                <template #icon>
                    <slot name="icon"></slot>
                </template>
                <slot></slot>
        ...
        <AddEdit
            ...
            :formData="formData"
            :data="formData"
            :pageTitle="$t('termination.add')"
            :successMessage="$t('termination.created')"
        />
    </div>
</template>
