<template>
    <div v-if="permsArray.includes('warnings_view') || permsArray.includes('admin')">
        <template v-if="customType == 'menu'">
            <a-menu-item @click="showAdd" key="warnings">
                <template #icon>
                    <slot name="icon"></slot>
                </template>
                <slot></slot>
        ...
        <AddEdit
            ...
            :formData="formData"
            :data="formData"
            :pageTitle="$t('warning.add')"
            :successMessage="$t('warning.created')"
        />
    </div>
</template>
