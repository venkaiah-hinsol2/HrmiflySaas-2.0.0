<template>
    <div v-if="permsArray.includes('resignations_view') || permsArray.includes('admin')">
        <template v-if="customType == 'menu'">
            <a-menu-item @click="showAdd" key="resignations">
                <template #icon>
                    <slot name="icon"></slot>
                </template>
                <slot></slot>
        ...
        <AddEdit
            ...
            :formData="formData"
            :data="formData"
            :pageTitle="$t('resignation.add')"
            :successMessage="$t('resignation.created')"
        />
    </div>
</template>
