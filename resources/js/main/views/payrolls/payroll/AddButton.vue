<template>
    <div v-if="permsArray.includes('payrolls_create') || permsArray.includes('admin')">
        <template v-if="customType == 'menu'">
            <a-menu-item @click="showAdd" key="payrolls">
                <template #icon>
                    <slot name="icon"></slot>
                </template>
                <slot></slot>
        ...
        <AddEdit
            ...
            :formData="formData"
            :data="formData"
            :pageTitle="$t('payroll.add')"
            :successMessage="$t('payroll.created')"
        />
    </div>
</template>
<script>
...
import AddEdit from "./AddEditQ.vue";
