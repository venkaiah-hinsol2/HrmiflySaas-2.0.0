<template>
    <div
        v-if="permsArray.includes('pre_payments_create') || permsArray.includes('admin')"
    >
        <template v-if="customType == 'menu'">
            <a-menu-item @click="showAdd" key="pre_payments">
                <template #icon>
                    <slot name="icon"></slot>
                </template>
                <slot></slot>
        ...
        <AddEdit
            ...
            :formData="formData"
            :data="formData"
            :pageTitle="$t('pre_payments.add')"
            :successMessage="$t('pre_payments.created')"
        />
    </div>
</template>
<script>
