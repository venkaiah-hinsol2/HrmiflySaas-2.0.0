<template>
    <div v-if="permsArray.includes('holidays_create') || permsArray.includes('admin')">
        <template v-if="customType == 'menu'">
            <a-menu-item @click="showAdd" key="holidays">
                <template #icon>
                    <slot name="icon"></slot>
                </template>
                <slot></slot>
        ...
        <AddEdit
            ...
            :formData="formData"
            :data="formData"
            :pageTitle="$t('holiday.add')"
            :successMessage="$t('holiday.created')"
        />
    </div>
</template>
<script>
