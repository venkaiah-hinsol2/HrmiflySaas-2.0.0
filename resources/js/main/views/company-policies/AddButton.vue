<template>
    <div
        v-if="
            permsArray.includes('company_policies_create') || permsArray.includes('admin')
        "
    >
        <template v-if="customType == 'menu'">
            <a-menu-item @click="showAdd" key="company_policies">
                <template #icon>
                    <slot name="icon"></slot>
                </template>
                <slot></slot>
        ...
        <AddEdit
            ...
            :formData="formData"
            :data="formData"
            :pageTitle="$t('company_policy.add')"
            :successMessage="$t('company_policy.created')"
        />
    </div>
</template>
<script>
