<template>
    <div v-if="permsArray.includes('departments_create') || permsArray.includes('admin')">
        <template v-if="customType == 'menu'">
            <a-menu-item @click="showAdd" key="departments">
                <template #icon>
                    <slot name="icon"></slot>
                </template>
                <slot></slot>
        ...
        <AddEdit
            ...
            :formData="formData"
            :data="formData"
            :pageTitle="$t('department.add')"
            :successMessage="$t('department.created')"
        />
    </div>
</template>
<script>
