<template>
    <div v-if="permsArray.includes('shifts_create') || permsArray.includes('admin')">
        <template v-if="customType == 'menu'">
            <a-menu-item @click="showAdd" key="shifts">
                <template #icon>
                    <slot name="icon"></slot>
                </template>
                <slot></slot>
        ...
        <AddEdit
            ...
            :formData="formData"
            :data="formData"
            :pageTitle="$t('shift.add')"
            :successMessage="$t('shift.created')"
        />
    </div>
</template>
<script>
