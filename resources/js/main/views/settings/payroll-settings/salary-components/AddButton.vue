<template>
    <div
        v-if="
            permsArray.includes('salary_components_view') || permsArray.includes('admin')
        "
    >
        <template v-if="customType == 'menu'">
            <a-menu-item @click="showAdd" key="salary_components">
                <template #icon>
                    <slot name="icon"></slot>
                </template>
                <slot></slot>
        ...
        <AddEdit
            ...
            :formData="formData"
            :data="formData"
            :pageTitle="$t('salary_component.add')"
            :successMessage="$t('salary_component.created')"
        />
    </div>
</template>
