<template>
    <div v-if="permsArray.includes('locations_view') || permsArray.includes('admin')">
        <template v-if="customType == 'menu'">
            <a-menu-item @click="showAdd" key="locations">
                <template #icon>
                    <slot name="icon"></slot>
                </template>
                <slot></slot>
        ...
        <AddEdit
            ...
            :formData="formData"
            :data="formData"
            :pageTitle="$t('location.add')"
            :successMessage="$t('location.created')"
        />
    </div>
</template>
