<template>
    <div v-if="permsArray.includes('leave_types_create') || permsArray.includes('admin')">
        <template v-if="customType == 'menu'">
            <a-menu-item @click="showAdd" key="leave_types">
                <template #icon>
                    <slot name="icon"></slot>
                </template>
                <slot></slot>
        ...
        <AddEdit
            ...
            :formData="formData"
            :data="formData"
            :pageTitle="$t('leave_type.add')"
            :successMessage="$t('leave_type.created')"
        />
    </div>
</template>
<script>
