<template>
    <div v-if="permsArray.includes('payers_view') || permsArray.includes('admin')">
        <template v-if="customType == 'menu'">
            <a-menu-item @click="showAdd" key="payers">
                <template #icon>
                    <slot name="icon"></slot>
                </template>
                <slot></slot>
        ...
        <AddEdit
            ...
            :formData="formData"
            :data="formData"
            :pageTitle="$t('payer.add')"
            :successMessage="$t('payer.created')"
        />
    </div>
</template>
