<template>
    <div v-if="permsArray.includes('expenses_create') || permsArray.includes('admin')">
        <template v-if="customType == 'menu'">
            <a-menu-item @click="showAdd" key="expenses">
                <template #icon>
                    <slot name="icon"></slot>
                </template>
                <slot></slot>
        ...
        <AddEdit
            ...
            :formData="formData"
            :data="formData"
            :pageTitle="$t('expense.add')"
            :successMessage="$t('expense.created')"
        />
    </div>
</template>
<script>
