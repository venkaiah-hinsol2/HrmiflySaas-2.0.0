<template>
    <div v-if="permsArray.includes('deposits_view') || permsArray.includes('admin')">
        <template v-if="customType == 'menu'">
            <a-menu-item @click="showAdd" key="deposits">
                <template #icon>
                    <slot name="icon"></slot>
                </template>
                <slot></slot>
        ...
        <AddEdit
            ...
            :formData="formData"
            :data="formData"
            :pageTitle="$t('deposit.add')"
            :successMessage="$t('deposit.created')"
        />
    </div>
</template>
