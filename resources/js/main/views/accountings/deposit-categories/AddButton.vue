<template>
    <div
        v-if="
            permsArray.includes('deposit_categories_view') || permsArray.includes('admin')
        "
    >
        <template v-if="customType == 'menu'">
            <a-menu-item @click="showAdd" key="deposit_categories">
                <template #icon>
                    <slot name="icon"></slot>
                </template>
                <slot></slot>
        ...
        <AddEdit
            ...
            :formData="formData"
            :data="formData"
            :pageTitle="$t('deposit_category.add')"
            :successMessage="$t('deposit_category.created')"
        />
    </div>
</template>
