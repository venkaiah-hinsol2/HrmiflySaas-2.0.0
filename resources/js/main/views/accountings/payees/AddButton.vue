<template>
    <div v-if="permsArray.includes('payees_view') || permsArray.includes('admin')">
        <template v-if="customType == 'menu'">
            <a-menu-item @click="showAdd" key="payees">
                <template #icon>
                    <slot name="icon"></slot>
                </template>
                <slot></slot>
        ...
        <AddEdit
            ...
            :formData="formData"
            :data="formData"
            :pageTitle="$t('payee.add')"
            :successMessage="$t('payee.created')"
        />
    </div>
</template>
