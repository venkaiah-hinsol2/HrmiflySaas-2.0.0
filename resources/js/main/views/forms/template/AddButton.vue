<template>
    <div v-if="permsArray.includes('forms_create') || permsArray.includes('admin')">
        <template v-if="customType == 'menu'">
            <a-menu-item @click="showAdd" key="forms">
                <template #icon>
                    <slot name="icon"></slot>
                </template>
                <slot></slot>
        ...
        <AddEdit
            ...
            :formData="formData"
            :data="formData"
            :pageTitle="$t('form.add')"
            :successMessage="$t('form.created')"
        />
    </div>
</template>
<script>
