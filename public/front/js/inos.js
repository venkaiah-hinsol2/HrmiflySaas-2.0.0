(function () {
    ...
    // show toaster model
    art.prototype.toaster = function (message, type, options) {
        tata[type](type.toUpperCase(), message, options);
    };
    // open model
    art.prototype.modal = function (selector, url, onLoad) {
        ...
        xhttp.onreadystatechange = function () {
            if (this.readyState == 4 && this.status == 200) {
                ...
                var contentScripts = modalContent.querySelectorAll("script");

                [].forEach.call(contentScripts, contentScript => {
                    var script = document.createElement("script");

                    if (contentScript.textContent) {
                        script.textContent = contentScript.textContent;
                    }

                    if (contentScript.src) {
                        script.src = contentScript.src;
                    }
                ...
                const arrClass = document.querySelectorAll(".modalClose");
                ...
                var xx = new bootstrap.Modal(document.getElementById(selector), {
                    keyboard: false
                });
    ...
    // Block screen function
    art.prototype.block = function (container, message) {
        if (message == undefined) {
            message = "Loading...";
        }

        // loader html if want to change the loader in future then just need to replace this html by new loader and write the css into inos.css
        var html = '<div class="lds-ellipsis"><div></div><div></div><div></div><div></div></div>';

        var querySelector = document.querySelector(container);
        querySelector.querySelector("div").classList.add('simple-block');

        //Create block container
        var screenBlock = document.createElement('div');
        screenBlock.setAttribute('class', 'screenBlock blockMsg blockElement');

        screenBlock.innerHTML = html;
        // Add block container
        querySelector.append(screenBlock);
    };

    // UnBlock screen function
    art.prototype.unblock = function (container) {
        var querySelector = document.querySelector(container);
        querySelector.querySelector("div").classList.remove('simple-block');
        querySelector.querySelector(".screenBlock").remove();
    };

    art.prototype.sendXhr = function (params) {
        var defaults = {
            toaster: {},
            data: {},
            file: false,
        ...
        if (options.toaster != '{}') {
            toastOptions = Object.assign(toastOptions, options.toaster);
        }

        // Check if beforeSend not defined
        if (typeof options.beforeSend != "function") {
            options.beforeSend = () => {
                // remove the previous errors
                if (container) {

                    container.querySelectorAll(".has-error").forEach(function (errorBlock) {
                        errorBlock.querySelector(".invalid-feedback").innerText = '';
                        errorBlock.querySelector(".is-invalid").classList.remove("is-invalid");
                        errorBlock.classList.remove("has-error");
                    });

                    var alert = container.querySelector(options.messageSelector);

                    if (alert) {
                        container.querySelector(options.messageSelector).innerHTML = '';
                    }

                    if (options.screenBlock) {
                        this.block(options.container);
                    }

                    if (options.disableButton) {
                        loading(options.buttonSelector);
                    }
                }
            }
        }

        // XmlHttpRequest complete block
        if (typeof options.complete != "function") {
            options.complete = (res) => {
                try {
                    if (options.screenBlock) {
                        this.unblock(options.container);
                    }

                    if (options.disableButton) {
                        unloading(options.buttonSelector)
                    }
                }
                catch (ex) {
                    console.log("Exception from complete block", ex)
                }
            }
        }
        ...
        if (typeof options.error != "function") {
            options.error = function (res, textStatus) {
                try {
                    ...
                    if (typeof response == "object") {
                        ...
                        var msg = "Something went wrong. Please try again after sometime.";

                        if (textStatus == "timeout") {
                            msg = "Connection timed out! Please check your internet connection";
                        }
                        showMessage(msg, "error", toastOptions);
                    }
                }
                catch (ex) {
                    console.log("Exception from error block", ex)
                }
            }
        }

        var showMessage = (msg, type, toastrOptions) => {
            ...
            if (options.messageLocation == "toast") {
                this.toaster(msg, type, toastrOptions);
            } else if (options.messageLocation == "none") {

            } else {
                var ele = container.querySelector(options.messageSelector);
                var html = '<div class="alert alert-' + classes[type] + '"><i class="fe fe-info"></i> ' + msg + '</div>';
                if (!ele) {
                    var messageSelector = options.messageSelector.replace('#', '');
                    var parentForm = container.querySelector(".form-group").parentElement;
                    parentForm.insertAdjacentHTML("beforeBegin", '<div id="' + messageSelector + '">' + html + '</div>');
                }
                else {
                    ele.innerHTML = html;
                }
        ...
        // Append file if there file true

        if (options.file == true) {
            try {
                var data = new FormData(document.querySelector(options.container));
                var keys = Object.keys(options.data);

                for (var i = 0; i < keys.length; i++) {
                    data.append(keys[i], options.data[keys[i]]);
                }

                options.data = data;
            }
            catch (ex) {
                console.log("Exception from file", ex)
            }
        } else {
            ...
            for (var key in optionData) {
                Array.isArray(optionData[key])
                    ? optionData[key].forEach(value => formData.append(key + '[]', value))
        ...
        try {
                    ...
                    if (response.status == "success") {
                        if (response.action == "redirect") {
                            if (options.redirect) {
                                ...
                                showMessage(message, "success", toastOptions);
                                window.location.href = response.url;
                            }
                        }
                        else {
                            if (typeof response.message != "undefined") {
                                showMessage(response.message, "success", toastOptions);
                            }
                        }

                        if (options.removeElements == true) {
                            container.querySelector(".form-group, button, input").remove();
                        }
                    }
            ...
            console.log(e, "from xmlhttprequest");
        }
        ...
        function loading(selector) {
            ...
            var text = '<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>Loading...';

            if (button.style.width < 20) {
                text = '<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span><span class="visually-hidden">Loading.</span>';
            }

            if (button.tagname !== "INPUT") {
                button.setAttribute("data-previous-text", button.innerHTML);
                button.innerHTML = text;
            }
            else {
                button.setAttribute("data-previous-text", button.value);
                button.value = text;
            }
        ...
        function unloading(selector) {
            var button = container.querySelector(selector);

            if (button) {
                var previewText = button.getAttribute("data-previous-text");

                if (button.tagname !== "BUTTON") {
                    button.innerHTML = previewText;
                }
                else {
                    button.value = previewText;
                }
                button.removeAttribute('disabled');
            }
        }

        function handleResponseFail(response) {
            if (typeof response.message != "undefined") {
                showMessage(response.message, "error", toastOptions);
            }

            if (typeof response.errors != "undefined") {
                var keys = Object.keys(response.errors);

                [].forEach.call(container.querySelectorAll(".has-error .invalid-feedback"), ele => ele.remove());

                [].forEach.call(container.querySelectorAll(".has-error"), ele => ele.classList.remove("has-error"));

                if (options.errorLocation == "field") {
                    for (var i = 0; i < keys.length; i++) {
                        // Escape dot that comes with error in array fields

                        var key = keys[i].replace(".", '\\.');
                        var ele = container.querySelector("[name='" + key + "']");

                        // Find by id if not found by name
                        if (!ele) {
                            ele = container.getElementById(key);
                        }

                        var grp = ele.closest(".form-group");
                        var grpInput = ele.closest(".form-control");
                        var helpBlock = grp.querySelector(".invalid-feedback");

                        if (helpBlock) {
                            helpBlock.remove();
                        }
                        //Check if there id another div inside the form group
                        var helpBlockContainer = grp.getElementsByTagName('div')[0];

                        if (!helpBlockContainer) {
                            // IF not another div inside the form group then assign group tp help block container
                            helpBlockContainer = grp;
                        } else {
                            if (helpBlockContainer.className == 'form-control-icon') {
                                helpBlockContainer = grp;
                            }
                        }
                        ...
                        helpBlock.setAttribute('class', 'invalid-feedback');

                        helpBlock.innerHTML = response.errors[keys[i]];

                        // Add help block in help block container
                        helpBlockContainer.append(helpBlock);

                        // Add has-error class to the form-group
                        grp.classList.add("has-error");
                        grpInput.classList.add("is-invalid");
                    }

                    if (keys.length > 0) {
                        var element = document.querySelector("[name='" + keys[0] + "']");
                        if (!element) {
                            document.querySelector("html, body").animate({ scrollTop: element.offset().top - 150 }, 200);
                        }
                    }
                }
                else {

                    // show errors as a list create the list
                    var errorMsg = "<ul>";
                    for (var i = 0; i < keys.length; i++) {
                    ...
                    var errorElement = container.querySelector(options.messageSelector);
                    var html = '<div class="alert alert-danger"><i class="fe fe-info"></i> ' + errorMsg + '</div>';

                    // Add the created list along with first form group
